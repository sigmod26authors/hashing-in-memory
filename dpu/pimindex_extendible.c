...
CHECK_LOCKS:
    ...
    if (ret == update_conflict) {
        ...
        return strict_insert(key, val, hash, tasklet_id);
    }
    else {
        return ret;
    }
...
int mapping_kernel() {
    ...
    /* cache locks in MRAM */
    uint32_t locks_per_buffer = WRAM_BUFFER_SIZE / sizeof(CBLock); /* TODO: ensure locks fit buffer precisely */
    CBLock* mr_bucket_locks = (CBLock*)((char*)mram_heap + dpu_args.keys_offs + 13 * MiB);
    ...
    for (uint32_t i = tasklet_id * keys_per_buffer; i < dpu_args.num_keys; i += NR_TASKLETS * keys_per_buffer) {
        ...
        for (uint32_t j = 0; j < num_keys; j++) {
            ...
            mutex_pool_lock(&chunk_hdr_locks_mtx, part); /* borrowing the mutex pool */
            wr_hist[part]++;
...
int (*kernels[NR_KERNELS])() = {initialization_kernel, load_kernel, insert_kernel, search_kernel,
                                mapping_kernel, kvmapping_kernel, mem_utilization_kernel};
