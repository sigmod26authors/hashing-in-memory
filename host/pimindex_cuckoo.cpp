...
void run_pimindex_cuckoo(void* workload, uint32_t load_keys, uint32_t insert_keys) {
    ...
    try {
        ...
        uint32_t insert_part_keys_offs = 2 * MAX_INDEX_SPACE + (2 * MiB);
        ...
        uint32_t insert_keys_offs = 2 * MAX_INDEX_SPACE + (2 * MiB);
        uint32_t est =
            ...
            for (uint32_t batch = 0; batch < load_batches; batch++) {
                ...
                for (uint32_t i = 0; i < NR_DPUS; i++) {
                    ...
                    for (uint32_t k = 0; k < keys; k++) {
                        PIMKey_t key =
                                load_workload[i * insert_part_keys_per_dpu + k];
                ...
                validation =
                        (total_padding == 0 && total_part_sizes == insert_keys);
                if (validation) {
                ...
                validation = true;
                for (uint32_t dpuid = 0; dpuid < NR_DPUS; dpuid++) {
                    if (dpu_params[dpuid].res_compl == 0) {
                        validation = false;
                        break;
                    }
                }
        ...
        search_part_keys_per_dpu =
            ((search_part_keys_per_dpu * (NR_DPUS - 1)) < search_keys) ?
            (search_part_keys_per_dpu) : (search_part_keys_per_dpu - 1);
        uint32_t size_search_part_keys_per_dpu = search_part_keys_per_dpu * KEY_SIZE;
        uint32_t search_part_keys_offs = 2 * MAX_INDEX_SPACE + (2 * MiB);
        ...
        uint32_t search_keys_offs = 2 * MAX_INDEX_SPACE + (2 * MiB);
