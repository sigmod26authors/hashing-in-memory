...
void run_three_level(void* workload, uint32_t load_keys,
    ...
    try {
        ...
        PRINT_TOP_RULE
        PRINT_INFO("PIM INDEX: Three-Level");
        PRINT_TOP_RULE
        ...
        PRINT_MSG("Max. chunks per tasklet: %lu", MAX_NUM_CHUNKS_PER_TASKLET);
        PRINT_MSG("Chunks bitmap length per tasklet: %lu", BITMAP_LEN_PER_TASKLET);
        PRINT_MSG("Chunks bitmap size per tasklet: %lu B",
                            BITMAP_LEN_PER_TASKLET * sizeof(uint32_t));
        ...
        /* transfer search result parameters to CPU */
        PRINT_MSG("Transfer search result parameters to CPU...");
        DPU_FOREACH(dpu_set, dpu, dpuid) {
            DPU_ASSERT(dpu_prepare_xfer(dpu, &dpu_params[dpuid]));
        }
        DPU_ASSERT(dpu_push_xfer(dpu_set, DPU_XFER_FROM_DPU,
        ...
        max_search_keys_per_dpu = 0;
        min_search_keys_per_dpu = (uint32_t)(-1);
        for (uint32_t i = 0; i < NR_DPUS; i++) {
            if (dpu_params[i].num_keys > max_search_keys_per_dpu) {
                max_search_keys_per_dpu = dpu_params[i].num_keys;
            }
            if (dpu_params[i].num_keys < min_search_keys_per_dpu) {
                min_search_keys_per_dpu = dpu_params[i].num_keys;
            }
        }
        max_search_keys_per_dpu_aligned = (max_search_keys_per_dpu % 2 == 0) ?
                                          (max_search_keys_per_dpu) :
                                          (max_search_keys_per_dpu + 1);
        ...
        uint32_t padding = 0;
        for (uint32_t dpuid = 0; dpuid < NR_DPUS; dpuid++) {
            uint32_t kvs = dpu_params[dpuid].num_keys;
            for (uint32_t kv = 0; kv < kvs; kv += 2) {
                if ((*dpu_input_buffers[dpuid])[kv + 1] == DEFAULT_VALUE) {
                    found++;
                }
                else {
                    not_found++;
            ...
            padding += dpu_params[dpuid].padding;
        }
        ...
        validation = (found == search_keys && not_found == 0);
        if (validation) {
        ...
        DPU_ASSERT(dpu_push_xfer(dpu_set, DPU_XFER_FROM_DPU,
                                 DPU_MRAM_HEAP_POINTER_NAME,
                                 MRAM_OUTPUT_OFFSET,
                                 2 * max_search_keys_per_dpu_aligned * KEY_SIZE,
                                 DPU_XFER_DEFAULT));
        t.stop();
        ...
        for (uint32_t dpuid = 0; dpuid < NR_DPUS; dpuid++) {
            uint32_t keys = dpu_params[dpuid].num_keys;
            for (uint32_t kv = 0; kv < (2 * keys); kv += 2) {
                if ((*dpu_input_buffers[dpuid])[kv + 1] == DEFAULT_VALUE) {
                    found++;
                }
                else {
